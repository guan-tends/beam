...
**Question for the maintainer**: should `handle_put` continue fanning out to other adapters after one fails, or fail-fast on first closed channel? **Plan choice: fail-fast** — atomic semantics, simpler caller reasoning.
...
**Question for the maintainer**: do we want fast-fail-with-backpressure (D3 strict) or graceful-with-timeout (D3 lenient)?
