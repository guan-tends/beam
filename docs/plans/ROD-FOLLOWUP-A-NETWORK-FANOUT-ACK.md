...
**Status**: LOCKED. Awaiting "begin" signal from the maintainer.
...
| D14 | **server-side integration caller impact**: zero, unless they explicitly opt-in via `put_quorum`. The existing `put()` API is preserved. |
...
| server-side integration callers depend on unbounded fanout (fire-and-forget for all writes) | None | `Node::put()` is unchanged. server-side integration continues to use `Node::put()`. Only callers who explicitly opt into `put_quorum` pay the latency cost. |
...
**Plan status**: LOCKED. Awaiting "begin" signal from the maintainer.
...
the maintainer's insight ("are oneshots more robust? do the redux/async-ack-and-drain fixes apply?") triggered the pivot. The architectural truth: **same drain envelope, different decoder**.
...
| D14 | server-side integration caller impact: zero unless opt-in |
