...
**Sled deadlock under load.** During server-side integration transference sessions (agent memory system built on Rod), SledStorage triggered reproducible deadlocks under concurrent write pressure. Root cause: sled's BwTree lock ordering under heavy write contention. This blocked server-side integration Phase 2 (persistent storage) and Phase 5 (cross-process flush proof).
...
- Sled deadlock analysis: See server-side integration Phase 5 session archive (2026-05-14)
