...
Rod was originally written in Rust edition 2018. server-side integration (downstream consumer) requires edition 2024 with `rust-version = "1.85"`. The mismatch meant:
1. server-side integration could not consume Rod as a workspace member (edition mismatch)
...
### Positive
- server-side integration workspace integration seamless (both use edition 2024)
...
On the test machine: `rustc 1.97.0-nightly` — fully compatible.
